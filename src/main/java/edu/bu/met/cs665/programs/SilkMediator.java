...
/**
 * This class represents silk Mediator programe
 * 
 * @author Sahad Osman
 *
 */
public class SilkMediator implements Mediator{
	 ...
	 * @param detergent
	 */
	public SilkMediator(WasherController washer, ColdWaterValve coldWaterValve,
	...
	@Override
	public void start() {
		System.out.println("Silk Program is started ");
		washer.start();
	}
	...
	public void wash() {
		...
		if(levelSensor.checkWaterLevel(level)&&
		 tempSensor.checkTemperature(30)){

		motor.startMotor();
		detergent.getDetergent(level);
		motor.rotateDrum(300);}
		motor.stopMotor();
		drainValve.open();
		rinse(level);
	}
